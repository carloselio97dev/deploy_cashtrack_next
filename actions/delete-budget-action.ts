...
import { Budget, ErrorResponseSchema, PasswoordValidationSchema, SuccessSchema } from "@/src/schemas";
...
export async function deleteBudget(budgetId:Budget['id'], prevState:ActionStateType,formData: FormData){
    const currentPassword = PasswoordValidationSchema.safeParse(formData.get('password'))
